...
PR bodyのプレースホルダーを実際の作業時間に置き換えるための正規表現パターンです。パターンを順に試し、最初に一致したパターンで置換します。一致するパターンが1つもない場合、そのPRは更新対象になりません（`replacement_patterns` は必須項目です）。

- `pattern`: マッチさせる正規表現（キャプチャグループ1を前置詞部分として保持できます）
- `replacement`: 置換テンプレート（`{hours}` が整形済みの時間文字列に展開されます）
- `hours_format`: 時間の表記形式（`"ja"` または `"en"`）
